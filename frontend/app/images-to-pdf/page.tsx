...
export default function ImagesToPdf() {
  ...
  // Clean up both the polling interval and any preview object URLs
  useEffect(() => {
    return () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (rejected > 0) {
      setErrorMessage(
        `${rejected} file${rejected > 1 ? "s" : ""} skipped — only PNG and JPG images are supported.`,
      );
    } else {
      setErrorMessage("");
    }
  ...
  const handleConversion = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/images-to-pdf`, {
        method: "POST",
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        fail("The server wouldn't take those images. Try a smaller batch.");
        return;
      }
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "SUCCESS") {
          ...
          fail("Couldn't build the PDF. One of the images may be corrupted.");
        }
      } catch (error) {
    ...
    <div className="paper-desk relative min-h-screen overflow-hidden px-5 py-14 sm:px-8">
      ...
      <main className="relative mx-auto flex max-w-2xl flex-col items-center">
        ...
        <div className="toon-panel w-full rounded-[26px] bg-[#FFF6E6] p-7 sm:p-9">
          <span className="inline-flex h-16 w-16 items-center justify-center rounded-2xl border-[3px] border-[#241F42] bg-[#FFDDB0] text-[#9A5A12]">
          ...
          <h1 className="display mt-5 text-3xl font-extrabold text-[#241F42] sm:text-4xl">Images to PDF</h1>
          <p className="mt-2 text-[15px] leading-relaxed text-[#5C5470]">
            Add a few PNGs or JPGs and we&rsquo;ll stack them into one PDF, in the order you add them.
          </p>
          ...
          >
            ...
            />
            <span className="display text-lg font-bold text-[#241F42]">Choose images</span>
            <span className="mt-1 text-sm text-[#5C5470]">PNG or JPG · add as many as you like</span>
          </label>
              ...
              <ul className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-4">
                {images.map((img, index) => (
                  <li key={img.previewUrl} className="relative">
                    <div className="thumb aspect-square overflow-hidden rounded-xl bg-white">
                    ...
                    <span className="mt-1 block truncate text-center text-xs font-semibold text-[#5C5470]">
                    ...
                    {!busy && (
                      <button
                        ...
                        className="remove-x absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-[#FFD3DE] text-xs font-bold text-[#A63A5C]"
                      >
                        ✕
                      </button>
                    )}
          ...
          {busy ? (
            <div className="mt-6 rounded-2xl border-[3px] border-[#241F42] bg-[#EDE4FA] px-5 py-4 text-center">
              ...
              <p className="mt-2 text-sm font-semibold text-[#5C5470]">
                {status === "uploading" ? "Sending your images over…" : "Stacking pages into a PDF…"}
              </p>
            </div>
          ) : status === "success" && downloadUrl ? (
            <div className="mt-6 rounded-2xl border-[3px] border-[#241F42] bg-[#C8F2D6] p-5 text-center">
              <h2 className="display text-xl font-bold text-[#1E6B44]">PDF ready</h2>
              <a
                href={downloadUrl}
            ...
            <>
              <button
                ...
                className="toon-btn display mt-6 w-full rounded-full bg-[#F5B24C] px-5 py-3 text-lg font-bold text-[#241F42] outline-none focus-visible:ring-4 focus-visible:ring-[#7ED6A5]"
              >
                Build the PDF
              </button>
...
//     try {
//       const response = await fetch("http://localhost:8000/api/v1/images-to-pdf", {
//         method: "POST",
//         body: formData,
//       });
...
//       <p className="text-gray-500 mb-8">Convert and compile multiple JPG or PNG images into a single PDF.</p>
...
//           </svg>
//           <span className="text-blue-600 font-semibold text-lg">Click to select images</span>
//           <span className="text-gray-400 text-sm mt-1">Supports PNG, JPG, JPEG</span>
//         </label>
//       </div>
...
//           >
//             {status === "error" ? "Conversion Failed - Try Again" : "Convert to PDF Now"}
//           </button>
//         ) : status === "success" && downloadUrl ? (
//           <a
//             href={downloadUrl}
...
//             {status === "uploading" ? "Uploading images..." : "Worker is compiling PDF..."}
//           </div>
//         )}
//       </div>
//     </div>
